...
def assert_tibbles_left_matches_right(A, B, tol=0, exact=False, sort_df=True, col_type=True):
    from pandas.testing import assert_frame_equal
    A_canonical = canonicalize_tibble(A, sort_df)
    B_canonical = canonicalize_tibble(B, sort_df)
    assert_frame_equal(A_canonical, B_canonical, check_exact=exact, atol=tol, check_dtype=col_type)
...
def compare_copies(a, b, tol=0, exact=False, sort_df=True, col_type=True):
    ...
    try:
        ...
        if isinstance(a, pd.DataFrame):
            try:
                assert_tibbles_left_matches_right(a, b, tol, exact, sort_df, col_type)
                return True
